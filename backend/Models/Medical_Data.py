...
class HealthData(Base):
    ...
    user_id = Column(UUID(as_uuid=True), ForeignKey("auth_users.id"), index=True)
    uploaded_by = Column(UUID(as_uuid=True), ForeignKey("auth_users.id"), nullable=False, index=True)
...
class MedicalAnalysis(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    report_id = Column(UUID(as_uuid=True), ForeignKey("health_reports.id"), nullable=False, index=True)
