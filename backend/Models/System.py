...
class AuditLog(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    actor_id = Column(UUID(as_uuid=True), ForeignKey("auth_users.id"), nullable=False, index=True)
    target_profile_id = Column(UUID(as_uuid=True), ForeignKey("Profile.id"), nullable=False, index=True)
